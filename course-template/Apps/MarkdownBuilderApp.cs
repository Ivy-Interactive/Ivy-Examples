        ...
        var leftCard = new Card(
            ...
            | (isFileSet.Value
                ? (object)(Layout.Horizontal().Align(Align.Left).Gap(2)
                    | Text.Block($"Editing: { (selectedPath.Value != null ? System.IO.Path.GetFileName(selectedPath.Value) : "") } or")
                    | new Button("Choose File From Directory").Icon(Icons.FolderOpen).HandleClick(() => { isSheetOpen.Value = true; })
                  )
                : (object)(Layout.Horizontal().Align(Align.Left).Gap(2)
                    | Text.Block("Enter your markdown code or")
                    | new Button("Choose File From Directory").Icon(Icons.FolderOpen).HandleClick(() => { isSheetOpen.Value = true; })
                  )
              )
