...
public class BenchmarkConnection : IConnection
{
    public string GetName() => "BenchmarkDb";

    public string GetNamespace() => typeof(BenchmarkConnection).Namespace!;
    ...
    {
        return typeof(BenchmarkDbContext)
            .GetProperties()
    ...
    public Task<(bool ok, string? message)> TestConnection(IConfiguration configuration)
    {
        try
        {
            ...
            var options = new DbContextOptionsBuilder<BenchmarkDbContext>().UseNpgsql(cs).Options;
    ...
    {
        server.Services.AddSingleton<BenchmarkDbContextFactory>();
    }

    public Secret[] GetSecrets()
    {
        return [new Secret("DB_CONNECTION_STRING", "PostgreSQL connection string")];
    }
}
