...
/// <summary>Sliplane API client for creating/deleting Tendril staging services.</summary>
public class SliplaneStagingClient
{
    ...
    /// <summary>Sanitize branch name: alphanumeric + hyphen, lowercase.</summary>
    ...
    public async Task<(int Deleted, int Failed)> DeleteAllServicesInProjectAsync(string apiToken, string projectId)
    {
        ...
        {
            if (string.IsNullOrEmpty(svc.Id)) { failed++; continue; }
            if (await DeleteServiceAsync(apiToken, projectId, svc.Id)) deleted++;
            else failed++;
        }
