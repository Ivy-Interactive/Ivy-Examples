...
public class SliplaneApiClient
{
    ...
    {
        var servers = await GetServersAsync(apiToken);
    ...
    public async Task<SliplaneVolume> CreateVolumeAsync(string apiToken, string serverId, string name)
    {
        ...
        if (response == null)
            throw new InvalidOperationException("Sliplane API did not respond when creating the volume.");
        ...
        if (string.IsNullOrWhiteSpace(volume?.Id))
            throw new InvalidOperationException("Sliplane returned an empty or invalid volume payload after volume creation.");
    ...
    private async Task<List<T>?> ParseArrayAsync<T>(HttpResponseMessage? response, string? arrayKey = null)
    {
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<List<T>>(root.GetRawText(),
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        }
        catch { return null; }
    }
